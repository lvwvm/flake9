====================================
 Writing Plugins For Flake9 2 and 3
====================================

Plugins have existed for |Flake9| 2.x for a few years. There are a number of
these on PyPI already. While it did not seem reasonable for |Flake9| to attempt
...
The **only two** breaking changes in |Flake9| 3.0 is the fact that we no
...
functions. On |Flake9| 2.x, to have an option parsed from the configuration
files that |Flake9| finds and parses you would have to do something like:
...
For |Flake9| 3.0, we have added *three* arguments to the
:meth:`~flake9.options.manager.OptionManager.add_option` method you will call
on the parser you receive:

- ``parse_from_config`` which expects ``True`` or ``False``

  When ``True``, |Flake9| will parse the option from the config files |Flake9|
  finds.

- ``comma_separated_list`` which expects ``True`` or ``False``

  When ``True``, |Flake9| will split the string intelligently and handle
  extra whitespace. The parsed value will be a list.

- ``normalize_paths`` which expects ``True`` or ``False``

  When ``True``, |Flake9| will:
...
The example from |Flake9| 2.x now looks like:
...
list of values from the user (e.g., ``--select E123,W503,F405``). |Flake9| 2.x
often forced users to parse these lists themselves since pep8 special-cased
certain flags and left others on their own. |Flake9| 3.0 adds
...
And |Flake9| will just return the list:
...
ensure that these paths are semi-normalized (the way |Flake9| 2.x used to
...
parser. The project has direct replications of the functions that |Flake9|
...
After releasing |Flake9| 2.6, handling standard-in became a bit trickier for
some plugins. |Flake9| 2.5 and earlier had started monkey-patching pep8's
