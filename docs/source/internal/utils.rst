...
|Flake9| has a few utility functions that it uses internally.
...
.. autofunction:: flake9.utils.parse_comma_separated_list

:func:`~flake9.utils.parse_comma_separated_list` takes either a string like
...
This function helps normalize any kind of comma-separated input you or |Flake9|
might receive. This is most helpful when taking advantage of |Flake9|'s
additional parameters to :class:`~flake9.options.manager.Option`.

.. autofunction:: flake9.utils.normalize_path
...
.. autofunction:: flake9.utils.normalize_paths

This function utilizes :func:`~flake9.utils.normalize_path` to normalize a
sequence of paths.  See :func:`~flake9.utils.normalize_path` for what defines a
normalized path.

.. autofunction:: flake9.utils.stdin_get_value
...
.. autofunction:: flake9.utils.is_windows
...
.. autofunction:: flake9.utils.is_using_stdin
...
|Flake9| to indicate we should read from stdin.

.. autofunction:: flake9.utils.filenames_from

When provided an argument to |Flake9|, we need to be able to traverse
...
Then they want us to check all of the files in the directory ``flake9/``. This
function will handle that while also handling the case where they specify a
file like:

.. code::

    $ flake9 flake9/__init__.py


.. autofunction:: flake9.utils.fnmatch
...
.. autofunction:: flake9.utils.parameters_for

|Flake9| analyzes the parameters to plugins to determine what input they are
...
.. autofunction:: flake9.utils.parse_unified_diff

To handle usage of :option:`flake9 --diff`, |Flake9| needs to be able
