...
Command-line options are often also set in configuration files for |Flake9|.
...
In |Flake9| 2, plugins received a :class:`optparse.OptionParser` instance and
...
Since |Flake9| 3 was rewritten from scratch, we decided to take a different
approach to configuration file parsing. Instead of needing to know about an
undocumented attribute that pep8 looks for, |Flake9| 3 now accepts a parameter
to ``add_option``, specifically ``parse_from_config`` which is a boolean
value.

|Flake9| does this by creating its own abstractions on top of :mod:`argparse`.
The first abstraction is the :class:`flake9.options.manager.Option` class. The
second is the :class:`flake9.options.manager.OptionManager`. In fact, we add
...
sufficient here. Telling |Flake9| that something is a comma-separated list
(e.g., ``comma_separated_list=True``) will handle this for you. |Flake9| will
return:
...
will receive an instance of :class:`~flake9.options.manager.OptionManager` so
to ease the transition we kept the same API as the
:class:`optparse.OptionParser` object. The only difference is that
:meth:`~flake9.options.manager.OptionManager.add_option` accepts the three
...
In |Flake9| 2, configuration file discovery and management was handled by
...
breaking |Flake9| again in the future, we have created our own discovery and
management.
...
- User files (files stored in a user's home directory or in the XDG directory
  ...
  user has a ``~/.flake9`` file, we will read that first.
...
- **New in 3.0.0** The user can specify ``--config <path-to-file>`` to so this
  file is the only configuration file used. This is a change from |Flake9| 2
...
of this function (|Flake9|'s authors believe) caused the confusion and
problems with pep8's 1.6 series. As such, |Flake9| has separated out
...
:class:`~flake9.options.config.ConfigFileFinder` object. This object needs to
...
:meth:`~flake9.options.config.ConfigFileFinder.local_config_files` to find
known local config files (and append the extra configuration files) and it
also provides :meth:`~flake9.options.config.ConfigFileFinder.local_configs`
...
:class:`~flake9.options.config.MergedConfigParser`. This requires the instance
of :class:`~flake9.options.manager.OptionManager` that the program is using,
the list of appended config files, and the list of extra arguments. This
object is currently the sole user of the
:class:`~flake9.options.config.ConfigFileFinder` object. It appropriately
initializes the object and uses it in each of

- :meth:`~flake9.options.config.MergedConfigParser.parse_cli_config`
- :meth:`~flake9.options.config.MergedConfigParser.parse_local_config`
- :meth:`~flake9.options.config.MergedConfigParser.parse_user_config`

Finally,
:meth:`~flake9.options.config.MergedConfigParser.merge_user_and_local_config`
takes the user and local configuration files that are parsed by
:meth:`~flake9.options.config.MergedConfigParser.parse_local_config` and
:meth:`~flake9.options.config.MergedConfigParser.parse_user_config`. The
main usage of the ``MergedConfigParser`` is in
:func:`~flake9.options.aggregator.aggregate_options`.

Aggregating Configuration File and Command Line Arguments
---------------------------------------------------------

:func:`~flake9.options.aggregator.aggregate_options` accepts an instance of
:class:`~flake9.options.maanger.OptionManager` and does the work to parse the
command-line arguments passed by the user necessary for creating an instance
of :class:`~flake9.options.config.MergedConfigParser`.
...
.. autofunction:: flake9.options.aggregator.aggregate_options

.. autoclass:: flake9.options.manager.Option
    :members: __init__, normalize, to_argparse

.. autoclass:: flake9.options.manager.OptionManager
    :members:
    :special-members:

.. autoclass:: flake9.options.config.ConfigFileFinder
    :members:
    :special-members:

.. autoclass:: flake9.options.config.MergedConfigParser
    :members:
    :special-members:
