...
def hook(ui, repo, **kwargs):
    """Execute Flake9 on the repository provided by Mercurial.
    ...
    strict = hgconfig.get("flake9", "strict", fallback=True)
...
def install():
    ...
    :rtype:
        bool
    :raises:
        flake9.exceptions.MercurialCommitHookAlreadyExists
    :raises:
        flake9.exceptions.MercurialQRefreshHookAlreadyExists
    """
    ...
    hgconfig.set("hooks", "commit", "python:flake9.main.mercurial.hook")
    hgconfig.set("hooks", "qrefresh", "python:flake9.main.mercurial.hook")

    if not hgconfig.has_section("flake9"):
        hgconfig.add_section("flake9")

    if not hgconfig.has_option("flake9", "strict"):
        hgconfig.set("flake9", "strict", False)

    with open(hgrc, "w") as fd:
        hgconfig.write(fd)

    print("mercurial hooks installed, for configuration options see")
    print("http://flake9.pycqa.org/en/latest/user/using-hooks.html")
