...
def install():
    ...
    :rtype:
        bool
    :raises:
        flake9.exceptions.GitHookAlreadyExists
    """
...
def copy_indexed_files_to(temporary_directory, lazy):
    # some plugins (e.g. flake9-isort) need these files to run their checks
...
def config_for(parameter):
    environment_variable = "flake9_{0}".format(parameter).upper()
    git_variable = "flake9.{0}".format(parameter)
