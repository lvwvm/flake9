...
SERIAL_RETRY_ERRNOS = {
    ...
    # > into this problem.
    # > (See also: https://gitlab.com/pycqa/flake9/issues/74)
    errno.ENOSPC,
...
class Manager(object):
    ...
    - Determining the parallelism of Flake9, e.g.:
    ...
    def __init__(self, style_guide, arguments, checker_plugins):
        ...
        :param style_guide:
            The instantiated style guide for this instance of Flake9.
        :type style_guide:
            flake9.style_guide.StyleGuide
        ...
        :type checker_plugins:
            flake9.plugins.manager.Checkers
        """
    ...
    def make_checkers(self, paths=None):
        ...
        def should_create_file_checker(filename, argument):
            ...
            # e.g, ``flake9 bin/script`` then we should run Flake9 against
    ...
    def run(self):
        ...
        https://gitlab.com/pycqa/flake9/issues/74) this also implements
        ...
        try:
            ...
            LOG.warning("Flake9 was interrupted by the user")
...
class FileChecker(object):
    ...
    def _extract_syntax_information(exception):
        ...
        if column > 0 and token and isinstance(exception, SyntaxError):
            ...
            column_offset = 1
            row_offset = 0
            # See also: https://gitlab.com/pycqa/flake9/issues/237
    ...
    def process_tokens(self):
        ...
        This can raise a :class:`flake9.exceptions.InvalidSyntax` exception.
        Instead of using this directly, you should use
        :meth:`flake9.checker.FileChecker.run_checks`.
        """
        parens = 0
...
def calculate_pool_chunksize(num_checkers, num_jobs):
    ...
    - This formula, while not perfect, aims to give each worker two batches of
      work.
    - See: https://gitlab.com/pycqa/flake9/merge_requests/156#note_18878876
    - See: https://gitlab.com/pycqa/flake9/issues/265
    """
