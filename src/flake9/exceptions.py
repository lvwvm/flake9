"""Exception classes for all of Flake9."""
from typing import Dict


class Flake9Exception(Exception):
    """Plain Flake9 exception."""


class EarlyQuit(Flake9Exception):
    """Except raised when encountering a KeyboardInterrupt."""


class ExecutionError(Flake9Exception):
    """Exception raised during execution of Flake9."""


class FailedToLoadPlugin(Flake9Exception):
    """Exception raised when a plugin fails to load."""

    FORMAT = 'Flake9 failed to load plugin "%(name)s" due to %(exc)s.'
...
class InvalidSyntax(Flake9Exception):
...
class PluginRequestedUnknownParameters(Flake9Exception):
...
class PluginExecutionFailed(Flake9Exception):
...
class HookInstallationError(Flake9Exception):
...
class GitHookAlreadyExists(HookInstallationError):
    ...
    def __init__(self, path):  # type: (str) -> None
        ...
        tmpl = (
            ...
            "Flake9 to install the hook, please remove the existing "
            "hook."
        )
...
class MercurialHookAlreadyExists(HookInstallationError):
    ...
    def __init__(self, path, value):  # type: (str, str) -> None
        ...
        tmpl = (
            ...
            "To convince Flake9 to install the hook, please remove the "
